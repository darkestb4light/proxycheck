...
int main(int argc, const char **argv)
{
  ...
  int           r_tmout = READ_TM_DEF, w_tmout = WRITE_TM_DEF, 
                ...
                redirect_cnt = 0, redirect_found = 0;
  ...
  for(unsigned int i = 1; i != argc; ++i)
  {
    for(unsigned int j = 0; j != MAX_ARGS; ++j)
    {
      if(strncmp(argv[i], *opt[j], 3) == 0){
        ...
        case 'D':
          ...
          break;
        case 'F':
          redirect = optfollow(argv, &i, &redirect_cnt);
          break;
        case 'H':
        ...
        case 'P':
          ...
          break;
        case 'R':
          if(strncmp("1", opt[14][1], 1) == 0) break; /* -r already exists */
        ...
        case 'p':
          ...
          break;
        case 'r':
          if(strncmp("1", opt[9][1], 1) == 0) break; /* -R already exists */
          if((request = (char *) malloc(REQ_BYTE_MAX)) == NULL){
            fprintf(stderr, "%s: ERR: %s: %s: %s\n", NAME, "request", 
                    "malloc", strerror(errno));
            exit(1);
        ...
        default: /* should never get here */
          fprintf(stderr, "%s: ERR: %s: %s (%c)\n", NAME, 
                  "argument/option found but unaccounted for", 
                  opt[j][0], c);
          exit(1);
        }
        optvalid = 1;
    ...
    if(!optvalid){
      fprintf(stderr, "%s: ERR: %s: %s\n", NAME, 
              "invalid argument or option", argv[i]);
      usage();
    }
    optvalid = 0;
  }
  ...
  {
    thisreq = req;
    while(thisreq->next != NULL) thisreq = thisreq->next;
  ...
  for(;;)
  {
    sd = initsock(serveraddr.sin_family, SOCK_STREAM, SOCK_PROTO, 
    ...
    */
    if(thisreq->connverb[0] != 0){
      ...
      if(reqbytes == -1){
        ...
        fprintf(stderr, "%s: %s%s: %s%s: %s%d: %s%s: %s%s\n", 
                "action=writereq", "connverb=", thisreq->connverb, 
                "domain=", thisreq->domain, "port=", thisreq->port, 
                "path=", thisreq->path, "ver=", thisreq->ver);
#if OUTPUT_REQ
      ...
      if(respbytes == -1){
        ...
        fprintf(stderr, "%s: %s: %s: %s: %s%s: %s%s: %s%d: ", ts, NAME, 
                ...
                skthost.s_name, "dport=", skthost.s_port);
        fprintf(stderr, "%s: %s%s: %s%s: %s%d: %s%s: %s%s\n",
                "action=readreq", "connverb=", thisreq->connverb, 
                "domain=", thisreq->domain, "port=", thisreq->port, 
                "path=", thisreq->path, "ver=", thisreq->ver);
#if OUTPUT_RESP
        fprintf(stderr, "%s: %s: %s: %s%s: %s%d: ", ts, NAME, "CONNECT", 
      ...
      if(status != 200){
        ...
        fprintf(stderr, "%s: %s%s: %s%s: %s%d: %s%s: %s%s: %s:%d\n", 
                "action=procresp", "connverb=", thisreq->connverb, "domain=",
        ...
        printf("%s: %s: %s: %s%s: %s%d: ", ts, NAME, "CONNECT", "dst=",       
              skthost.s_name, "dport=", skthost.s_port);
        printf("%s: %s%s: %s%s: %s%d: %s%s: %s%s: %s:%d\n", "action=procresp",
              "connverb=", thisreq->connverb, "domain=", thisreq->domain, 
    ...
    */
    ...
    if(reqbytes == -1){
      ...
      fprintf(stderr, "%s: %s%s: %s%s: %s%d: %s%s: %s%s\n", "action=writereq",     
              "verb=", thisreq->verb, "domain=", thisreq->domain, "port=", 
    ...
    case 300 ... 399:
      ...
      if(redirect_cnt){
        ...
        if(redirect_found){
          printf("%s: %s: %s: %s%s: %s%d: ", ts, NAME, "STANDARD", "dst=",
                skthost.s_name, "dport=", skthost.s_port);
          if(getlocation(response, respbuf, resploc, RESP_LOC_BUF)){
    ...
    case 403: /* possible proxy ACL; keep separate from other HTTP 4XX */
      printf("%s: %s: %s: %s: %s%s: %s%d: ", ts, NAME, "WARN", 
            "STANDARD", "dst=", skthost.s_name, "dport=", skthost.s_port);
      printf("%s: %s%s: %s%s: %s%d: %s%s: %s%s: %s:%d: %s\n", 
            "action=procresp", "verb=", thisreq->verb, "domain=", 
    ...
    case 407:
      ...
      fprintf(stderr, "%s: %s: %s: %s: %s%s: %s%d: ", ts, NAME, "ERR", 
              "STANDARD", "dst=", skthost.s_name, "dport=", skthost.s_port);
      fprintf(stderr, "%s: %s%s: %s%s: %s%d: %s%s: %s%s: %s:%d: %s\n", 
              "action=procresp", "verb=", thisreq->verb, "domain=", 
              thisreq->domain, "port=", thisreq->port, "path=", 
              thisreq->path, "ver=", thisreq->ver, "status", status,
              RESP_MSG_407);
