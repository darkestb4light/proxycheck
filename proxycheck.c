...
{
  char 	      host[REQ_PATH_SZ]= {0}, *tmphost = NULL, 
              *path = NULL, reqport[6] = {0};
  const char  *hdr = reqopt->hdr;
  int 	      portoverride = 0, len;
  ...
  strncpy(host, reqopt->tmpurl, REQ_PATH_SZ);
  len = strlen(host);

  if(len >= REQ_PATH_SZ)
    host[REQ_PATH_SZ-1] = '\0';
  else
    host[len] = '\0';

  if(strstr(reqopt->tmpurl, "http://") != NULL){
    ...
    tmphost = host;
    tmphost += 7;
    if((path = strchr(tmphost, '/')) == NULL){
      strncpy(req->path, reqopt->path, REQ_PATH_SZ);
      strncpy(req->domain, tmphost, HOST_SZ);
      strncpy(req->path, reqopt->path, REQ_PATH_SZ);
    }else{
      strncpy(req->path, path, REQ_PATH_SZ);
      strncpy(req->domain, tmphost, HOST_SZ);
      tmphost = strchr(req->domain, '/');
      *tmphost = '\0';
    }
#if 1 /* force REQ_HTTP_PORT_DEF (for now) */
    req->port = REQ_HTTP_PORT_DEF;
#else
    if(reqopt->port != -1)
      req->port = reqopt->port;
    else
    req->port = REQ_HTTP_PORT_DEF;
#endif
  }else if(strstr(reqopt->tmpurl, "https://") != NULL){
    req->port = REQ_HTTPS_PORT_DEF;
...
#if 1 /* force REQ_HTTPS_PORT_DEF (for now) */
    req->port = REQ_HTTPS_PORT_DEF;
#else
    if(reqopt->port != -1)
      req->port = reqopt->port;
    ...
    strncpy(req->domain, host, HOST_SZ);
    if(reqopt->port != -1){
      req->port = reqopt->port;
      if(strncasecmp(req->scheme, "https://", strlen(req->scheme)+1) == 0){
        strncpy(req->scheme, "https://", REQ_SCHEME_SZ);
        strncpy(req->connverb, "CONNECT", REQ_VERB_SZ);
#if CONN_FORCE_GET_DEF_VERB               
        strncpy(req->verb, "GET", REQ_VERB_SZ);
#else
        strncpy(req->verb, reqopt->verb, REQ_VERB_SZ);
#endif
      }else if(strncasecmp(reqopt->verb, "CONNECT", strlen(req->verb)+1) == 0){
        strncpy(req->connverb, "CONNECT", REQ_VERB_SZ);
        strncpy(req->verb, "GET", REQ_VERB_SZ);
      }else{
        memset(req->connverb, 0, REQ_VERB_SZ);
  ...
  if(req->connverb[0] != 0){
    ...
    strncat(req->connreq, ":", (REQ_CONN_SZ-len)-1);
    len = strlen(req->connreq);
    strncat(req->connreq, reqport, (REQ_CONN_SZ-len)-1);
    len = strlen(req->connreq);
    strncat(req->connreq, " ", (REQ_CONN_SZ-len)-1);
    len = strlen(req->connreq);
    ...
    if(portoverride){
      ...
      req->httpreq[REQ_SZ-1] = '\0';
      len = strlen(req->httpreq);
      strncat(req->httpreq, reqport, (REQ_SZ-len)-1);
    }
    len = strlen(req->httpreq);
    strncat(req->httpreq, req->path, (REQ_SZ-len)-1);
    len = strlen(req->httpreq);
...
#if DEBUG_BUILDREQ
  fprintf(stderr, "\nDEBUG: request: %s:\n%s%d\n%s%d\n%s%d\n%s%s\n", 
          "options passed", "REQ HDR HOST: ", reqopt->do_host_hdr, 
          "REQ HDR CONN: ", reqopt->do_conn_hdr, "REQ PORT: ", reqopt->port, 
          "REQ METHOD: ", reqopt->verb);
  fprintf(stderr, "%s%s\n%s%s\n%s%s\n%s%s\n%s%s\n", 
          "REQ SCHEME: ", reqopt->scheme, "REQ TMP URL: ", reqopt->tmpurl, 
          "REQ PATH: ", reqopt->path, "REQ VER: ", reqopt->ver, 
          "REQ HDRS: ", reqopt->hdr);
  fprintf(stderr, "\nDEBUG request: %s:\n%s%d\n%s%s\n%s%s\n", 
          "object built", "REQ PORT: ", req->port, "REQ CONN METHOD: ", 
          req->connverb, "REQ METHOD: ", req->verb);
  fprintf(stderr, "%s%s\n%s%s\n%s%s\n%s%s\n", 
          "REQ SCHEME: ", req->scheme, "REQ DOMAIN: ", req->domain, 
          "REQ PATH: ", req->path, "REQ VER: ", reqopt->ver); 
  fprintf(stderr, "%s%s\n%s%s\n%s%s\n%s%s\n", 
          "REQ HDR HOST: ", req->host_hdr, "REQ HDR CONN: ", req->conn_hdr, 
          "REQ (CONN): ", req->connreq, "REQ (Resource): ", req->httpreq);
#if DEBUG_FORCE_EXIT
  exit(1);
#endif
#endif
            ...
            socklen_t sa_len)
{
  ...
  if((sd = socket(sa_fam, type, proto)) == -1){
    fprintf(stderr, "%s: ERR: %s: %s\n", NAME, "socket", strerror(errno));
    exit(1);
  }

  if(connect(sd, (struct sockaddr *) &sa_addr, sa_len) == -1){
    fprintf(stderr, "%s: ERR: %s: %s\n", NAME, "connect", strerror(errno));
    exit(1);
  }
...
void logevents(char *logfile, int lmode, FILE *lstream)
{
  ...
  case LOG_IOFBF:
    ...
    break;
  default:
    fprintf(stderr, "%s: ERR: %s: %d\n", NAME, "log mode unknown", lmode);
    exit(1);
  }

#if 0 /* skip close(2) for streams; defer to dup2() */
  close(STDIN_FILENO);
  close(STDOUT_FILENO);
  close(STDERR_FILENO);
#endif
...
{
  int   i = 1, http_redirect[MAX_REDIRECTS] = {301, 302, 303, 307},
...
{
  int i, flag = 0;
  char httpverb[MAX_VERBS][REQ_VERB_SZ] = {"CONNECT", "GET", 
                                          "HEAD", "OPTIONS"};
...
#if DEBUG_PROCRESP
  fprintf(stderr, "DEBUG: response: processing:\n%s\n", response);
#if DEBUG_FORCE_EXIT
  exit(1);
#endif
#endif
...
#if DEBUG_READREQ
  fprintf(stderr, "DEBUG: request: reading:\n%s\n", response);
#if DEBUG_FORCE_EXIT
  exit(1);
#endif
#endif
...
#if DEBUG_WRITEREQ
  fprintf(stderr, "DEBUG: request: sending:\n%s\n", request);
#if DEBUG_FORCE_EXIT
  exit(1);
#endif
#endif
